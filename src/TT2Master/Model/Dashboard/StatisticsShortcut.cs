...
{
    [ShortcutAttribute(16, "Statistics", false)]
    public class StatisticsShortcut : DashboardShortcut
    {
        public override int ID { get; set; } = 16;

        public override string Header { get; set; } = AppResources.Statistics;
        ...
        public StatisticsShortcut(INavigationService navigationService, IPageDialogService dialogService) : base(navigationService, dialogService)
        {
            ...
            {
                var result = await _navigationService.NavigateAsync(NavigationConstants.ChildNavigationPath<DashboardPage, StatisticsPage>());
