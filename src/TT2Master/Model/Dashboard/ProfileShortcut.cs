...
{
    [ShortcutAttribute(6, "Profile", true)]
    public class ProfileShortcut : DashboardShortcut
    {
        public override int ID { get; set; } = 6;

        public override string Header { get; set; } = AppResources.Profile;
        ...
        public ProfileShortcut(INavigationService navigationService, IPageDialogService dialogService) : base(navigationService, dialogService)
        {
            Destination = typeof(ArtOptImageGridPage).Name;

            ItemTappedAction = new DelegateCommand(async () =>
            {
                var result = await _navigationService.NavigateAsync(NavigationConstants.ChildNavigationPath<DashboardPage, ClanMemberDetailPage>(), new NavigationParameters()
                {
                    { "member", App.Save.ThisPlayer?.PlayerName },
                    { "profile", ""},
                });
